...
int venta_seleccionarVentaCodigo(void* this,char* parametro)
{
    ...
    if(this != NULL)
    {
        codigoVenta = venta_getCodigo(this);
...
int venta_unidadesCodigo(void* this,char* parametro)
{
    int retorno = 0;
    char* codigo;
    ...
    if(this != NULL)
    {
        codigo = venta_getCodigo(this);

        if(!strcasecmp(codigo,parametro))//Comparo el sueldo del elemento contra el numero ingresado
        {
            retorno = venta_getCantidad(this);
        }
    }
    return retorno;
}
